...
class Utility {
	...
	public static function send_mail( $to, $subject, $content ) {
		...
		//Email Template
		$email_template = wp_normalize_path( \WP_AFRAEX::$plugin_path . '/templates/email.php' );
		if ( trim( \WP_AFRAEX::$Template_Engine ) != "" ) {
			$template = wp_normalize_path( path_join( get_template_directory(), '/wp-afraex/email.php' ) );
